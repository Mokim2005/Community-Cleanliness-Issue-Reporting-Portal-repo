    ...
    <div className="min-h-screen flex items-center justify-center bg-[#050b18] px-4 relative overflow-hidden">
      ...
      <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-10 rounded-[2.5rem] w-full max-w-md shadow-2xl relative z-10 transition-all duration-500">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 font-medium">Please enter your details to login</p>
        </div>

        <form onSubmit={handleLogIn} className="space-y-6">
          {/* Email Input */}
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-300 ml-1 italic">Email Address</label>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center ml-1">
              <label className="text-sm font-semibold text-slate-300 italic">Password</label>
              <button type="button" className="text-xs text-secondary hover:underline transition-all">Forgot password?</button>
            </div>
            <div className="relative group">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full py-4 bg-secondary text-secondary-content font-black rounded-2xl shadow-xl shadow-secondary/20 hover:shadow-secondary/40 hover:-translate-y-1 transition-all duration-300 active:scale-95 flex justify-center items-center ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
          >
            {loading ? <span className="loading loading-spinner loading-md"></span> : "LOGIN"}
          </button>
        </form>

        <div className="divider before:bg-white/10 after:bg-white/10 text-slate-500 text-[10px] my-8 uppercase tracking-[0.3em] font-black">Secure Login</div>
        ...
        <p className="mt-10 text-center text-slate-400 font-medium">
          New to CleanCity?{" "}
          <Link to="/register" className="text-secondary font-black hover:underline underline-offset-8 decoration-2 transition-all">
            Join the mission
          </Link>
        </p>
      </div>
